...
function Signup() {
    ...
    const [emailHelperText, setEmailHelperText] = useState('*이메일을 입력해주세요.');
    const [passwordHelperText, setPasswordHelperText] = useState('*비밀번호를 입력해주세요.');
    const [checkPasswdHelperText, setCheckPasswdHelperText] = useState('*비밀번호를 한번 더 입력해주세요.');
    const [nicknameHelperText, setNicknameHelperText] = useState('*닉네임을 입력해주세요.');
    ...
    useEffect(() => {
        setValidEmail(false);
        if(email.length === 0) {setEmailHelperText('*이메일을 입력해주세요.')}
        else if(!isValidEmail(email) || email.length < 6) {setEmailHelperText('*올바른 이메일 주소 형식을 입력해주세요.\n(예: example@example.com)')}
        else {
            setEmailHelperText('');
        }
    }, [email]);
    ...
    useEffect(() => {
        ...
        if(passwd.length === 0) {setPasswordHelperText('*비밀번호를 입력해주세요.')}
        else if(!isValidPassword(passwd)){setPasswordHelperText('*비밀번호는 8자 이상, 20자 이하이며, 대문자, 소문자, 숫자, 특수문자를 각각 최소 1개 포함해야 합니다.')}
        else if(passwd !== checkPasswd){setPasswordHelperText('*비밀번호가 다릅니다.')}
        else {
    ...
    useEffect(() => {
        ...
        if(nickname.length === 0) {setNicknameHelperText('*닉네임을 입력해주세요.')}
        else if(!isValidNickname(nickname) && nickname.length < 10){setNicknameHelperText('*띄어쓰기를 없애주세요.');}
        else if(!isValidNickname(nickname) && nickname.length > 10){setNicknameHelperText('*닉네임은 최대 10자까지 작성가능합니다.');}
        else {
    ...
    const handleNicknameBlur = async () => {
        if(nicknameHelperText === '' && await isDuplicate(nickname, 'nickname')){
    ...
    const handleSignup = async () => {
        ...
        await fetch(`${config.API_URL}/auth/signup`, {
            ...
            if(!res.ok) {throw new Error('save data failed')};
            return res.json();
        })
    ...
    return(
        <SignupArticle>
            ...
            <InputField placeholder={'이메일을 입력하세요'} type={'email'} name={'email'} inputListener={e => setEmail(e.target.value)} onBlur={handleEmailBlur}/>
            <HelperText label={emailHelperText}/>
            <InputLabel label={'비밀번호*'}/>
            <InputField placeholder={'비밀번호를 입력하세요'} type={'password'} name={'passwd'} inputListener={e => setPasswd(e.target.value)}/>
            <HelperText label={passwordHelperText}/>
            <InputLabel label={'비밀번호 확인*'}/>
            <InputField placeholder={'비밀번호를 한번 더 입력하세요'} type={'password'} name={'checkPasswd'} inputListener={e => setCheckPasswd(e.target.value)}/>
            <HelperText label={checkPasswdHelperText}/>
            <InputLabel label={'닉네임*'}/>
            <InputField placeholder={'닉네임을 입력하세요'} type={'text'} name={'nickname'} inputListener={e => setNickname(e.target.value)} onBlur={handleNicknameBlur}/>
            <HelperText label={nicknameHelperText}/>
            <SetMainButton label={'회원가입'} onClick={handleSignup} valid={signupOk}/>
